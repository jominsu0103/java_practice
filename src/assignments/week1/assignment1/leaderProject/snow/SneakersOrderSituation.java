...
public class SneakersOrderSituation {
    public static void main(String[] args) {
        ...
        while (scanner.hasNext()) {
            ...
            if (response.equals("끝")) break;

            try {
                String[] responseArray = response.split(",");
                CustomerRank customerLevel = CustomerRank.valueOf(responseArray[0]);
                ...
                switch (customerLevel) {
                    case VIP:
                        ...
                        break;
                    case GOLD:
                        customer = new GoldCustomer(customerName, cache, sneakerModelName, isCustomerLikeDelivery);
                        break;
                    case SILVER:
                        customer = new SilverCustomer(customerName, cache, sneakerModelName, isCustomerLikeDelivery);
                        break;
                }
        ...
        for (Customer customer : customers) {
            try {
                ...
                SneakerPackageInfo sneakerPackageInfo = staff.orderNikeSneakersToDeliverManager(modelName, deliveryManager);
