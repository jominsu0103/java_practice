...
public class LeaderAssignmentTest {
    public static void main(String[] args) {
        ...
        for (Order order : orders) {
            try {
                Customer customer = order.getCustomer().orElseThrow(() -> {
                    throw new RuntimeException("고객이 누락 되었습니다.");
                });
                if (validStatus.contains(order.getStatus()))
                    throw new RuntimeException(customer + "님의 주문이 아직 주문 처리 중입니다.");
                List<OrderItem> orderItems = order.getOrderItems().orElseThrow(() -> {
                    throw new RuntimeException(customer + "님의 주문 아이템들이 누락 되었습니다.");
                });
